...
if(isset($postdata) && !empty($postdata))
{
  // Extract the data.
    ...
    $data = $con->prepare("SELECT * FROM film WHERE nom = :nom");
    $data->bindValue(':nom', $nom, PDO::PARAM_STR);
    $data->execute();

    if($data->rowCount())
    {
        $errorEmail = 'Film deja existant.';
        $error = true;
    ...
    if(!isset($error))
    {
        $insert = $con->prepare("INSERT INTO film (nom, duree, genre, acteur, realisateur, date, note, description) VALUES (:nom, :duree, :genre, :acteur, :realisateur, :date, :note, :description)");
